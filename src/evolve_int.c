...
// prool, Jan 2026: this utility 'evolve' transformation to internal function.
// This need for cygwin

// all external variables renamed, f.e. i -> e_i, stack -> e_stack
...
#define DEBUG_MODE  TRUE // by prool
...
int e_nearby_distance = 2;	// for nearby evos
int e_day_of_year = 180;	// for seasons
int e_water_crop_distance = 4;	// for crops that require water
...
struct map_t e_world[MAP_WIDTH][MAP_HEIGHT];	// world map grid
struct map_t *e_land = NULL;	// linked list of land tiles
crop_data *e_crop_table = NULL;	// crop hash table
sector_data *e_sector_table = NULL;	// sector hash table
FILE *e_outfl = NULL;	// file for saving data


// these are the arguments to shift_tile() to shift one tile in a direction, e.g. shift_tile(tile, e_shift_dir[dir][0], e_shift_dir[dir][1]) -- NUM_OF_DIRS
const int e_shift_dir[][2] = {
	{ 0, 1 },	// north
	{ 1, 0 },	// east
	{ 0, -1},	// south
	{-1, 0 },	// west
...
//void basic_mud_log(const char *format, ...) { } // prool
...
void evolve_one(struct map_t *tile) {
	...
	if (become == NOTHING && (evo = get_evo_by_type(tile->sector_type, EVO_NEAR_SECTOR))) {
		if (sect_within_distance(tile, evo->value, e_nearby_distance, TRUE)) {
			become = evo->becomes;
		}
	}
	...
	if (become == NOTHING && (evo = get_evo_by_type(tile->sector_type, EVO_NOT_NEAR_SECTOR))) {
		...
		// formerly: if (!sect_within_distance(tile, evo->value, e_nearby_distance, TRUE)) {
			become = evo->becomes;
		}
	}
	...
	if (become == NOTHING && (evo = get_evo_by_type(tile->sector_type, EVO_NEAR_SECTOR_FLAG))) {
		if (sect_flagged_within_distance(tile, evo->value, e_nearby_distance, TRUE)) {
			become = evo->becomes;
		}
	}
	...
	if (become == NOTHING && e_water_crop_distance > 0 && !check_crop_water(tile) && !number(0, 1)) {
...
int spread_one(struct map_t *tile) {
	...
	LL_FOREACH(GET_SECT_EVOS(st), evo) {
		...
		for (dir = 0; dir < NUM_2D_DIRS; ++dir) {
			...
			if (!(to_room = shift_tile(tile, e_shift_dir[dir][0], e_shift_dir[dir][1]))) {
...
int evolve_main(int p1, int p2, int p3, int p4) {
	struct map_t *tile;
	int num;

	printf("%s prool debug: evolve internal\n", ptime()); // prool
	
	//empire_srandom(time(0)); // prool fool
	
	e_nearby_distance = p1;
	e_day_of_year = p2;
	e_water_crop_distance = p3;	// may be 0 to ignore unwatered crops
	
	if (DEBUG_MODE) {
		printf("%s prool debug evolve. ", ptime()+4);
		printf("Using nearby distance of: %d. ", e_nearby_distance);
		printf("Using day of year: %d. ", e_day_of_year);
		printf("Using water crop distance: %d. ", e_water_crop_distance);
	}
	...
	index_boot_sectors();
	if (DEBUG_MODE) {
		printf("Loaded: %d crops. ", HASH_COUNT(e_crop_table));
		printf("Loaded: %d sectors. ", HASH_COUNT(e_sector_table));
	}
	
	load_base_map();
	
	if (DEBUG_MODE) {
		LL_COUNT(e_land, tile, num); 
		printf("Loaded: %d e_land tiles. ", num);
	}
	
	// open file for writing
	if (!(e_outfl = fopen(EVOLUTION_FILE TEMP_SUFFIX, "wb"))) {
...
void parse_crop(FILE *fl, crop_vnum vnum) {
	...
	if ((find = find_crop(vnum))) {
		log("prool debug WARNING: Duplicate crop vnum #%d (1)", vnum); // prool fool
		// but have to load it anyway to advance the file
	}
	else {
		HASH_ADD_INT(e_crop_table, vnum, crop);
	}
...
void parse_sector(FILE *fl, sector_vnum vnum) {
	...
	if ((find = find_sect(vnum))) {
		printf("prool debug WARNING: Duplicate sector vnum #%d (2)\n", vnum); // prool fool
		// but have to load it anyway to advance the file
	}
	else {
		HASH_ADD_INT(e_sector_table, vnum, sect);
	}
...
void load_base_map(void) {
	...
	for (x = 0; x < MAP_WIDTH; ++x) {
		for (y = 0; y < MAP_HEIGHT; ++y) {
			e_world[x][y].vnum = (y * MAP_WIDTH) + x;
			e_world[x][y].island_id = NO_ISLAND;
			e_world[x][y].sector_type = BASIC_OCEAN;
			e_world[x][y].base_sector = BASIC_OCEAN;
			e_world[x][y].natural_sector = BASIC_OCEAN;
			e_world[x][y].crop_type = NOTHING;
			e_world[x][y].affects = NOBITS;
			e_world[x][y].flags = NOBITS;
			e_world[x][y].sector_time = 0;
			e_world[x][y].next = NULL;
		}
	}
	...
	for (y = 0; y < header.height && !feof(fl); ++y) {
		for (x = 0; x < header.width && !feof(fl); ++x) {
			...
			if (y < MAP_HEIGHT && x < MAP_WIDTH) {
				e_world[x][y].island_id = store.island_id;
				e_world[x][y].sector_type = store.sector_type;
				e_world[x][y].base_sector = store.base_sector;
				e_world[x][y].natural_sector = store.natural_sector;
				e_world[x][y].affects = store.affects;
				e_world[x][y].flags = store.misc;
				e_world[x][y].sector_time = store.sector_time;
				e_world[x][y].crop_type = store.crop_type;
				
				if (e_world[x][y].sector_type != BASIC_OCEAN) {
					if (last_land) {
						last_land->next = &e_world[x][y];
					}
					else {
						e_land = &e_world[x][y];
					}
					last_land = &e_world[x][y];
				}
			}
		}
	}
...
void write_tile(struct map_t *tile, sector_vnum old) {
	...
	fwrite(&dat, sizeof(struct evo_import_data), 1, e_outfl);
}
...
bool check_crop_water(struct map_t *tile) {
	...
	else if (sect_flagged_within_distance(tile, SECTF_FRESH_WATER, e_water_crop_distance, TRUE)) {
...
bool check_near_evo_sector(struct map_t *tile, int type, struct evolution_data *for_evo) {
	...
	LL_FOREACH(GET_SECT_EVOS(sect), evo) {
		...
		if (evo->type == type && sect_within_distance(tile, evo->value, evo->type == EVO_NOT_NEAR_SECTOR ? e_nearby_distance : 1.5, TRUE)) {
			found = TRUE;
			break;
		}
	}
...
bool check_near_evo_sector_flagged(struct map_t *tile, int type, struct evolution_data *for_evo) {
	...
	LL_FOREACH(GET_SECT_EVOS(sect), evo) {
		...
		if (evo->type == type && sect_flagged_within_distance(tile, evo->value, evo->type == EVO_NOT_NEAR_SECTOR_FLAG ? e_nearby_distance : 1.5, TRUE)) {
			found = TRUE;
			break;
		}
	}
...
int count_adjacent(struct map_t *tile, sector_vnum sect, bool count_original_sect) {
	...
	for (iter = 0; iter < NUM_2D_DIRS; ++iter) {
		to_room = shift_tile(tile, e_shift_dir[iter][0], e_shift_dir[iter][1]);
...
int count_adjacent_flagged(struct map_t *tile, sbitvector_t with_flags, bool count_original_sect) {
	...
	for (iter = 0; iter < NUM_2D_DIRS; ++iter) {
		if (!(to_room = shift_tile(tile, e_shift_dir[iter][0], e_shift_dir[iter][1]))) {
			continue;	// no room
		}
...
crop_data *find_crop(any_vnum vnum) {
	crop_data *crop;
	HASH_FIND_INT(e_crop_table, &vnum, crop);
	return crop;
}
...
sector_data *find_sect(any_vnum vnum) {
	sector_data *sect;
	HASH_FIND_INT(e_sector_table, &vnum, sect);
	return sect;
}
...
int season(struct map_t *tile) {
	...
	if (e_day_of_year < 6 * 30) {	// first half of year
		if (half_y >= (e_day_of_year + 1) * a_slope) {	// first winter line
			return northern ? TILESET_WINTER : TILESET_SUMMER;
		}
		else if (half_y >= (e_day_of_year - 89) * b_slope) {	// spring line
		...
		if (half_y >= (e_day_of_year - 360) * -a_slope) {	// second winter line
			return northern ? TILESET_WINTER : TILESET_SUMMER;
		}
		else if (half_y >= (e_day_of_year - 268) * -b_slope) {	// autumn line
...
struct map_t *shift_tile(struct map_t *origin, int x_shift, int y_shift) {
	...
	if (get_coord_shift(MAP_X_COORD(origin->vnum), MAP_Y_COORD(origin->vnum), x_shift, y_shift, &x_coord, &y_coord)) {
		return &e_world[x_coord][y_coord];
	}
	return NULL;
}
...
//static unsigned long e_seed; // prool
